...
mashupApp.service('cacheService', function ($http, $q, $log, utility, detectService, sessionService) {
    ...
    isCacheStale('mashCacheStart', minutesInOneWeek).then(function (result) {
        ...
        if (result) {
            ...
            var logObject = utility.getLogObject("mashCasheDelete", "MashupCoreUI", "cacheService", "root of cacheService", "Stale-Cache", sessionService);
    ...
    return {
        ...
        getData: function (cacheName, schema, webApiUrl, staleMinutes) {
            ...
            isCacheStale(cacheName, staleMinutes).then(function (cacheIsStale) {
                ...
                var webApiAvailable = detectService.detect("http://localhost:50000/api/HeartBeat/", "MashupCoreApi");
                ...
                if (cacheIsStale && webApiAvailable) {
                    ...
                    $http.get(webApiUrl, { withCredentials: true })
                        .success(function (data) {
                            ...
                            try {
                                // add data to cache
                                dbCache.put(schema, data); //.done(function () { alert("done"); }).fail(function () { alert("fail"); });
                                // updateCacheAge
                                updateCacheAge(cacheName);
                            }
                            catch (err) {
                                
                                var logObject = utility.getLogObject("mashCasheDelete", "MashupCoreUI", "cacheService", "getData", "Error", sessionService);
                                $log.error(err, logObject);
                                indexedDB.deleteDatabase('mashCacheDB');
                                $log.log("IndexedDB error on updating a cache. Deleted database to allow new schema.", logObject);
                                //alert("cacheService Try Block: IndexedDB error on updating a cache. Deleted database to allow new schema.");
                                //TODO: if the above try block isn't seen for a long time then remove it.
                            }
                    ...
                    .error(function () {
                        ...
                        // alert("Web Api Error");
                        detectService.failed("http://localhost:50000/api/HeartBeat/", "MashupCoreApi");
                        getCache(cacheName).then(function (data) {
                            // async alert()
