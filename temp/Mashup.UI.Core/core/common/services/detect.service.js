...
mashupApp.service('detectService', function ($http, $q, $log, $interval, $filter, $rootScope, utility, sessionService) {
    "use strict";
        ...
        setInterval(function () {
            ...
            (function wait() {
                if (logHeartbeatReady) {
                    ...
                    var logObject = utility.getLogObject("detectManagement:Truncate", "Mashup.UI.Core", "detectService", "anonymous:setInterval", null, sessionService);
    ...
    var recordHeartBeatResult = function (webApiName, result, data) {
        ...
        var logObject = utility.getLogObject("HeartBeat", "Mashup.UI.Core", "detectService", "recordHeartBeatResult", result, sessionService);
        if (result) {
            logObject.msg = msg;
    ...
    var addToCodeBlueList = function (item) {
        if (!_.where($rootScope.codeBlueList, { 'id': item.id }).length) {
            ...
            if (!codeBlueMonitor) {
                ...
                var logObject = utility.getLogObject("StartCodeBlueMonitor", "Mashup.UI.Core", "detectService", "addToCodeBlueList", "Starting", sessionService);
    ...
    var startCodeBlueMonitor = function () {
        ...
        codeBlueMonitor = $interval(function () {
            ...
            if ($rootScope.codeBlueList.length === 0) {
                ...
                var logObject = utility.getLogObject("CodeBlueMonitor", "Mashup.UI.Core", "detectService", "startCodeBlueMonitor", "ended", sessionService);
                ...
                _.each($rootScope.codeBlueList, function (item) {
                    ...
                    $http.get(item.url, { withCredentials: true })
                        .success(function (data) {
                            ...
                            $rootScope.codeBlueList = _.filter($rootScope.codeBlueList, function (x) { return x.id !== item.id; });
                        })
                        .error(function () {
                            result = false;
                            item.detected = result;
    ...
    var recordCodeBlueResult = function (webApiName, result, data) {
        ...
        var logObject = utility.getLogObject("CodeBlueMonitor", "Mashup.UI.Core", "detectService", "recordCodeBlueResult", result, sessionService);
    ...
    var canDetectHeartBeat = function (heartBeatUrl, webApiName) {
        ...
        try {
            ...
            var logObject = utility.getLogObject("CodeBlueMonitor", "Mashup.UI.Core", "detectService", "canDetectHeartBeat", "Error", sessionService);
            logObject.webApiName = webApiName;
    ...
    return {
        ...
        failed: function (heartBeatUrl, webApiName) {
            ...
            var logObject = utility.getLogObject("CodeBlueMonitor", "Mashup.UI.Core", "detectService", "detectService.failed", false, sessionService);
            logObject.webApiName = webApiName;
            logObject.heartBeatUrl = heartBeatUrl;
